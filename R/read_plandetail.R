...
call_eprd_api_plandetail <- function(base_uri, planid){
  ...
  resp_plandetail <- req |>
    ...
    req_url_path_append("cds-au/v1/energy/plans") |>
    
    req_url_path_append(planid) |>
...
plan_to_df <- function(base_uri, planid){
  ...
  if (any(str_detect(names(plandetail_list), "gasContract"))) {
...
read_eprd_plan <- function(base_uri, planid){
  ...
  plan <- plan_to_df(base_uri, planid) %>% 
    mutate(contract = list(tidy_details_to_df(contract))
    )
